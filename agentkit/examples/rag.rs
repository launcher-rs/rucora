...
use agentkit::rag::{Citation, chunk_text, index_chunks, retrieve};
use agentkit::retrieval::{ChromaVectorStore, InMemoryVectorStore};
use agentkit_core::embed::EmbeddingProvider;
use agentkit_core::retrieval::{VectorQuery, VectorStore};
...
async fn test_in_memory_rag() -> anyhow::Result<()> {
    ...
    let documents = vec![
        (
            "doc1",
            "Rust 是一种系统编程语言，注重安全性和性能。它通过所有权系统保证内存安全。",
        ),
        (
            "doc2",
        ...
        (
            "doc3",
            "机器学习是人工智能的一个子集，使用统计技术让计算机从数据中学习。",
        ),
        (
            "doc4",
            "深度学习是机器学习的一个分支，使用多层神经网络来模拟人类学习过程。",
        ),
        (
            "doc5",
...
async fn test_rag_with_agent() -> anyhow::Result<()> {
    ...
    let knowledge_base = vec![
        (
            "rust_intro",
            "Rust 是一门系统编程语言，专注于安全性、并发性和性能。它适用于构建可靠且高效的软件。",
        ),
        (
            "rust_ownership",
            "Rust 的所有权系统是其核心特性。每个值都有一个所有者，当所有者离开作用域时，值会被丢弃。",
        ),
        (
            "rust_borrowing",
            "Rust 的借用系统允许你引用数据而不获取所有权。使用 & 符号创建引用。",
        ),
        (
            "rust_lifetimes",
            "Rust 的生命周期确保引用在使用期间始终有效。它们是编译时检查的。",
        ),
        (
            "rust_concurrency",
            "Rust 的类型系统保证了并发安全。'无数据竞争' 是其核心保证之一。",
        ),
    ];
...
async fn test_document_management() -> anyhow::Result<()> {
    ...
    let tech_docs = vec![
        (
            "microservices",
            "微服务架构是一种将单一应用程序开发为一组小型服务的方法。每个服务运行在自己的进程中，并通过轻量级机制（通常是 HTTP）进行通信。",
        ),
        (
            "containers",
            "容器是一种轻量级的虚拟化技术，可以打包代码及其所有依赖项，确保软件在任何环境中都能一致运行。Docker 是最流行的容器平台。",
        ),
        (
            "kubernetes",
            "Kubernetes 是一个开源的容器编排平台，用于自动化部署、扩展和管理容器化应用程序。它最初由 Google 设计。",
        ),
        (
            "devops",
            "DevOps 是开发（Development）和运维（Operations）的结合，旨在缩短系统开发周期并提供高质量的持续交付。",
        ),
        (
            "ci_cd",
            "CI/CD（持续集成/持续交付）是一种通过自动化构建、测试和部署来频繁交付代码变更的软件开发实践。",
        ),
    ];
    ...
    let results: Vec<_> = store
        .search(VectorQuery::new(query_vector).with_top_k(10))
        .await?;
    ...
    info!(
        "平均每个文档：{:.1} 个块",
        total_chunks as f64 / tech_docs.len() as f64
    );
...
fn build_context(citations: &[Citation]) -> String {
    citations
        .iter()
        .map(|cite| cite.text.as_deref().unwrap_or("").to_string())
        .collect::<Vec<_>>()
        .join("\n\n")
}
