#[cfg(not(feature = "mcp"))]
fn main() {
    eprintln!("This example requires feature 'mcp'. Try: cargo run -p agentkit --example mcp_demo --features mcp");
}
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mcp_url = std::env::var("MCP_URL").unwrap_or_else(|_| "http://127.0.0.1:8000/mcp".to_string());
